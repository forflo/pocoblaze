...
/* Store and fetch */
const char *fetch_reg_from_sY = "10011";
const char *fetch_reg_from_k  = "10111";
const char *store_sX_at_sY    = "11100";
const char *store_sX_at_k     = "11111";
...
#define instruction_count    33  /* total instruction set */
...
opcode_t op[MAX_LINE_COUNT]; /* operaton array to save info for each line */
...
const char *instruction_set[] =
{
    ...
    "ADDRESS",  /* 28 */
    "FLIP",     /* 29 */
    "TEST",      /* 30 */ /* added new instruction; TEST, FETCH and Store */
    "FETCH",    /* 31 */
    "STORE",     /* 32 */
};
...
void test_instructions(void) {
    ...
    for (i = 0; i < line_count; i++) {
        if (op[i].instruction != NULL) {
            ...
            if ((j < 26) || (j > 28) || (j == 30) || (j == 31) || (j == 32)) {
                address++;
            }
        } else{
            op[i].address = address;    /* This is a comment line*/
        }
    }
}
    ...
    for (i = 0; i < line_count; i++) {
        if (op[i].instruction != NULL) {
            ...
            case 32:    /* STORE */
                ...
                if (j == 31) {
                    kptr = fetch_reg_from_k;
                    sptr = fetch_reg_from_sY;
                }
                if (j == 3) {
                    kptr = load_k_to_x_id;
                    sptr = load_y_to_x_id;
                ...
                if (j == 8) {
                    kptr = addcy_k_to_x_id;
                    sptr = addcy_y_to_x_id;
                }
                if (j == 9) {
                    kptr = sub_k_to_x_id;
                    sptr = sub_y_to_x_id;
                ...
                if (j == 10) {
                    kptr = subcy_k_to_x_id;
                    sptr = subcy_y_to_x_id;
                }
                /* Edit: TEST */
                if (j == 30) {
                    kptr = test_sX_k;
